...
public class TC_002_MIB_BudgetRejected_Test extends BaseUtilityClass{
	
	@Test(retryAnalyzer = RetryanalyserClass.class)	
	public void MIB_budgetRejected_test() throws Exception
	{
		...
		webaction.select_dd_by_value(MIB_Quote_Approval.getBudget_Approval_dd(), "0");
		String comment1 = filclass.readData_TestData("MIB_Data",16,2);
		MIB_Quote_Approval.getBudget_Approval_comment().sendKeys(comment1);
		MIB_Quote_Approval.getBudget_Approval_submit_button().click();
		if(MIB_Quote_Approval.getRejected_text().isDisplayed())
		{
			String path=concatenate+webaction.screenshot_MIB("MIB Rejected");
			test.info("MIB order Quote Budget Rejected",MediaEntityBuilder.createScreenCaptureFromPath(path).build());
			Reporter.log("Step:2 MIB order Quote Budget Rejected Successfully! and Verification Passed",true);
		}
		else
		{
			String path=concatenate+webaction.screenshot_MIB("MIB is not approved");
			test.info("MIB order Quote Budget is not Rejected",MediaEntityBuilder.createScreenCaptureFromPath(path).build());
			Reporter.log("Step:2 MIB order Quote Budget Not Rejected and Verification Failed",true);
		}
   }
}
