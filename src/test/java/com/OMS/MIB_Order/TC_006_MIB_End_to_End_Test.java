...
public class TC_006_MIB_End_to_End_Test extends BaseUtilityClass {
	
		@Test (retryAnalyzer = RetryanalyserClass.class)	
		public void MIB_end_to_end_test() throws Exception
		{
			...
			webaction.scrollBy_bottom();
			Thread.sleep(1000);
			webaction.select_dd_by_value(Shipment_Status.getShipping_Status_Label(), "6");
			webaction.select_dd_by_value(Shipment_Status.getshipping_comment2_Label(), "The customer changed her/his mind");
			Shipment_Status.getDate_Time_Picker_Label().sendKeys("2023-01-02 17:25:16");
			Shipment_Status.getsubmit().click();
			Thread.sleep(2000);
			webaction.scrollBy_bottom();
			Thread.sleep(2000);
			if(Shipment_Status.getReturn_text().isDisplayed())
			{
				String path=concatenate+webaction.screenshot_MIB("MIB Order Return details Updated");
				test.info("MIB Order Return details Updated",MediaEntityBuilder.createScreenCaptureFromPath(path).build());
				Reporter.log("Step:7 MIB Order Qty Return Done Successfully and Verification Passed",true);
			}
			else
			{
				String path=concatenate+webaction.screenshot_MIB("MIB Order Return details Not Updated");
				test.info("MIB Order Return details is Not Updated",MediaEntityBuilder.createScreenCaptureFromPath(path).build());
				Reporter.log("Step:7 MIB Order Qty Return NOT Done and Verification Failed",true);
			}
			webaction.scrollBy_bottom();
			Thread.sleep(500);
			if(Shipment_Status.getpending_button().isDisplayed())
			{
				String path=concatenate+webaction.screenshot_MIB("Required finance Fill");
				test.info("Required To Fill Finance Info for MIB Order For Return Qty",MediaEntityBuilder.createScreenCaptureFromPath(path).build());
				Reporter.log("Step:8  Required To Fill Updated Finance Info for MIB Order Return Qty and Verification Passed",true);
			}
			else
			{
				String path=concatenate+webaction.screenshot_MIB("Not Required finance Fill");
				test.info("Button Not visible Required To Fill Finance Info for MIB Order For Return Qty",MediaEntityBuilder.createScreenCaptureFromPath(path).build());
				Reporter.log("Step:8 Required To Fill Updated Finance Info for MIB Order Return Qty and Verification Failed",true);
			}
			Shipment_Status.getupdate_fin_info().click();
			Thread.sleep(3000);
			//Now switch to next window
			webaction.getWindowHandle();
			webaction.getWindowHandles();
			Update_Finance_Details.getInvoice_Number_Label().sendKeys(Invoice_Number);
			Update_Finance_Details.getInvoice_Date_label().sendKeys(Keys.ENTER);
			Update_Finance_Details.getInvoice_Attachmente_label().sendKeys(pdf);
			Update_Finance_Details.geteway_Bill_No_label().sendKeys(eway_Bill_No);
			...
			Thread.sleep(1000);
			if(Shipment_Status.getsuccess_green_flag().isDisplayed())
			{
				String path=concatenate+webaction.screenshot_MIB("Filled finance Fill");
				test.info("Filled Finance Info for MIB Order For Return Qty",MediaEntityBuilder.createScreenCaptureFromPath(path).build());
				Reporter.log("Step:9 Filled Updated Finance Info for MIB Order Return Qty and Verification Passed",true);
			}
			else
			{
				String path=concatenate+webaction.screenshot_MIB("Not Filled finance Fill");
				test.info("Button Not Visible Filled Finance Info for MIB Order For Return Qty",MediaEntityBuilder.createScreenCaptureFromPath(path).build());
				Reporter.log("Step:9 NOT Filled Updated Finance Info for MIB Order Return Qty and Verification Failed",true);
			}
			String GRN_Number=Shipment_Status.getGRN_Number_text().getText();
			System.out.println("GRN Number is: "+GRN_Number);
			test.info("GRN Number : "+GRN_Number+"_"+"Order Number :"+MIB_Order_ID);
