...
public class TC_002_SO_Create_So_with_KYC_without_L1_Test extends BaseUtilityClass {

	String testCaseDescription = "Create_So_with_KYC_without_L1 ";
	@Test
	public void Create_So_with_KYC_without_L1_Test() {
		
		String currentClassName = this.getClass().getSimpleName();
		setTestCaseAndDescriptionInReport("SO Negative", currentClassName, testCaseDescription);

		String SKU_Name=getFileLibClass().getDataFromExcell("Create_So_with_kyc_L1_pending", "SKU_Name", IAutoconstant.EXCEL_TestData);
		String SKU_QTY=getFileLibClass().getDataFromExcell("Create_So_with_kyc_L1_pending", "SKU_Qty", IAutoconstant.EXCEL_TestData);
		String Shipping_price=getFileLibClass().getDataFromExcell("Create_So_with_kyc_L1_pending", "Shipping_price", IAutoconstant.EXCEL_TestData);
		String customer_name=getFileLibClass().getDataFromExcell("Create_So_with_kyc_L1_pending", "cutomer name", IAutoconstant.EXCEL_TestData);
		String customer_email=getFileLibClass().getDataFromExcell("Create_So_with_kyc_L1_pending", "customer email", IAutoconstant.EXCEL_TestData);
